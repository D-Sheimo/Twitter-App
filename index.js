...
if (user) {
    ...
    profile.innerHTML = `<div class="profileUser">
        ...
        <div class="followCnt">
            <h4>
                <span style="color: black;">${formatNumber(followingCount)}</span> <span style="font-weight: 500;">Following</span>
            </h4>
            <h4>
                <span style="color: black;">${formatNumber(followerCount)}</span> <span style="font-weight: 500;">Followers</span>
            </h4>
        </div>
    </div>`;
        ...
        tweetsContainer.innerHTML += `
            <div class="mainName-1">
                <p class="userInfo-1">
                    ...
                    <span class="date">${user.tweets[i].timestamp.split(' ')[0]}</span>
                </p>
...
tweetsTimeline.innerHTML = `<div class="timeline-date">
    <div class="datebtn">
        <button id="userButton" class="newPage-btn" onclick="location.href='${selectedUser === 'user1' ? 'index.html?user=user2' : 'index.html?user=user1'}'">${selectedUser === 'user1' ? 'Bill Gates' : 'Elon Musk'}</button>
